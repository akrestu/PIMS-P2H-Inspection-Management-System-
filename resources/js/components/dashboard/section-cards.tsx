...
export function SectionCards({ metrics }: { metrics: Metrics }) {
    const p2hRatio =
        metrics.total_unit_aktif > 0
            ? Math.round(
                  (metrics.total_p2h_hari_ini / metrics.total_unit_aktif) * 100,
              )
            : 0;

    const tlRatio =
        metrics.total_p2h_hari_ini > 0
            ? Math.round(
                  (metrics.unit_tidak_layak_hari_ini /
                      metrics.total_p2h_hari_ini) *
                      100,
              )
            : 0;
    ...
    const cards: CardConfig[] = [
        ...
        },
        {
            ...
            badgeVariant:
                metrics.total_p2h_hari_ini > 0 ? 'default' : 'secondary',
            progress: p2hRatio,
        ...
        },
        {
            ...
            icon: AlertTriangle,
            accent:
                metrics.unit_tidak_layak_hari_ini > 0
            ...
            badgeVariant:
                metrics.unit_tidak_layak_hari_ini > 0 ? 'secondary' : 'default',
            progress: tlRatio,
        ...
        },
        {
            ...
            icon: ShieldAlert,
            accent:
                metrics.critical_tidak_layak > 0
            ...
            badgeVariant:
                metrics.critical_tidak_layak > 0 ? 'destructive' : 'default',
                    ...
                    return (
                        <Tooltip key={card.title}>
                            <TooltipTrigger asChild>
                                <Card
                                    className={`relative overflow-hidden bg-gradient-to-br ${card.accent} cursor-default border-border/60 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md`}
                                >
                                    <CardHeader className="pt-4 pb-2">
                                        <div className="flex items-start justify-between">
                                            ...
                                            <div
                                                className={`rounded-xl p-2.5 ${card.iconBg}`}
                                            >
                                                <Icon
