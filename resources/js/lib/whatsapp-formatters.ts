...
function deptSuffix(row: {
    ...
    return row.jenis_unit === 'Light Vehicle' && row.department
        ? ` (${row.department})`
        : '';
}

/** Group LV rows by department, returns sorted Map<deptName, rows[]>. */
function groupLvByDept<
    T extends { jenis_unit: string; department: string | null },
...
function pushCollapsed(lines: string[], items: string[], max = 5): void {
    ...
    if (items.length > max) {
        lines.push(`  _...dan ${items.length - max} unit lainnya_`);
    }
}
...
function pushJoined(lines: string[], names: string[], max = 2): void {
    ...
    if (remaining > 0) {
        lines.push(`_...dan ${remaining} unit lainnya_`);
    }
}

// ── PA Monitoring Formatter ───────────────────────────────────────────────────

export function formatPaReport(
    unitData: UnitPA[],
    summary: PaSummary,
    filters: PaFilters,
): string {
    ...
    lines.push(
        filters.jenis_unit
            ? `${rangeStr} · ${jenisLabel(filters.jenis_unit)}`
            : rangeStr,
    );
    ...
    if (summary.no_data_count > 0) {
        lines.push(`Belum ada data: ${summary.no_data_count} unit`);
    }
    ...
    if (bdUnits.length > 0) {
        ...
        for (const u of bdUnits) {
            ...
            if (u.downtime_hours > 0) {
                parts.push(`downtime ${u.downtime_hours.toFixed(0)} jam`);
            }

            if (u.bd_days > 0) {
                parts.push(`rusak ${u.bd_days} hari`);
            }
    ...
    if (belowThreshold.length > 0) {
        lines.push('');
        lines.push(
            `*Perlu Perhatian — PA di bawah ${threshold}% (${belowThreshold.length} unit)*`,
        );
    ...
    const normalUnits = unitData
        .filter(
            (u) =>
                u.current_status === 'operation' &&
                (u.actual_pa === null || u.actual_pa >= threshold),
        )
    ...
    if (normalUnits.length > 0) {
        ...
        const items = normalUnits.map((u) => {
            ...
            if (u.actual_pa !== null) {
                parts.push(`PA ${u.actual_pa}%`);
            }

            if (u.compliance_pa !== null) {
                parts.push(`P2H ${u.compliance_pa}%`);
            }
...
export function formatP2hReport(
    ...
    if (notFilled.length === 0) {
        ...
        const lvNotFilled = notFilled.filter(
            (r) => r.jenis_unit === 'Light Vehicle',
        );
        const busNotFilled = notFilled.filter(
            (r) => r.jenis_unit !== 'Light Vehicle',
        );
    ...
    if (filledLayak.length > 0) {
        ...
        const lvLayak = filledLayak.filter(
            (r) => r.jenis_unit === 'Light Vehicle',
        );
        const busLayak = filledLayak.filter(
            (r) => r.jenis_unit !== 'Light Vehicle',
        );
...
export function formatP2hHistoryReport(
    ...
    const sorted = [...matrix].sort(
        (a, b) => a.compliance_pct - b.compliance_pct,
    );
    const poor = sorted.filter((r) => r.compliance_pct < 70);
    const fair = sorted.filter(
        (r) => r.compliance_pct >= 70 && r.compliance_pct < 90,
    );
    ...
    if (poor.length > 0) {
        ...
        const items = poor.map(
            (r) =>
                `${unitLabel(r)}${deptSuffix(r)} — ${r.compliance_pct}% · ${r.filled_days}/${r.total_days} hari`,
        );
        pushCollapsed(lines, items, 10);
    }
        ...
        const items = good.map((r) => {
            const detail =
                r.compliance_pct === 100
                    ? 'sempurna'
                    : `${r.compliance_pct}% · ${r.filled_days}/${r.total_days} hari`;
